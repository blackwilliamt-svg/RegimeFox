...
class MonthlyBudget:
    """Compute-unit ceiling for Birdeye's monthly allowance.

    Birdeye's free Standard tier is 30,000 compute units per month and an OHLCV
    call costs 35, so the whole month is roughly 857 calls. That is small enough
    that a careless historical pull exhausts it in one run, which is why every
    Birdeye call checks this first and refuses rather than silently failing
    mid-backfill.
    """
    ...
    def can_spend(self, units: int) -> bool:
        with self._lock:
            if self.limit <= 0:
                return True  # 0 means "unmetered tier, do not enforce"
    ...
    def stats(self) -> dict[str, int | float]:
        with self._lock:
            pct = (100.0 * self.spent / self.limit) if self.limit > 0 else 0.0
            return {
                "limit": self.limit,
                "spent": self.spent,
                "remaining": max(0, self.limit - self.spent) if self.limit > 0 else -1,
